...
void Simulation::Init(Scene* scene)
{
    ...
    for (uint32_t loadedMeshID : loadedMeshIDs)
    {
        ...
        scene->Transforms[mScene->Instances[parentInstanceID].TransformID].parentID = -1;

        AddMeshInstance(mScene, loadedMeshID, &childInstanceID);
        //set childInstanceID to be relative to parentInstanceID
        scene->Transforms[scene->Instances[childInstanceID].TransformID].parentID = scene->Instances[parentInstanceID].TransformID;
        scene->Transforms[childInstanceID].Translation = glm::vec3(3.0f, 1.0f, 0.0f);
        scene->Transforms[childInstanceID].RotationOrigin = -scene->Transforms[childInstanceID].Translation;
    ...
    uint32_t newInstanceID;
    AddMeshInstance(mScene, loadedMeshID, &newInstanceID);
    uint32_t newTransformID = scene->Instances[newInstanceID].TransformID; scene->Transforms[newTransformID].Scale = glm::vec3(1.0f / 50.0f);
    }
}
...
void Simulation::Update(float deltaTime)
{
    ...
    mScene->Transforms[mSpinningTransformID].Rotation *= glm::angleAxis(
